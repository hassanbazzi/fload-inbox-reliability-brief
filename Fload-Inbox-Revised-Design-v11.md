# FLO-1355 — revised design v11

17 September 2026 · Corrected design for independent review · Implementation paused

**This version applies the v10 review corrections directly.** It adds an honest exhausted-verification exit, handles invalidated inspection edits without inventing cleanup evidence, scopes release recovery to an actual scheduling decision, and makes cyclic history references erasable in one authorized transaction. The accepted architecture remains the foundation. Permanent tickets, immutable revisions, exact approvals, stable children, typed provider boundaries and the existing Actions/BullMQ architecture remain the design. This is a documentation specification; it is not migrated DDL, a passing implementation or a deployment.
...
This appendix carries forward the accepted contracts that the v11 execution and Usage corrections do not replace. It describes the proposed design, not shipped behavior. v11 is authoritative for scheduling, inspection and binding, evidence/finality, retry and reconciliation, reopening, Usage, and privileged erasure. Earlier predicates for those areas must not be reinstated from this appendix.
...
2. Every issued cleanup DELETE is independently `request_final`; no unfinished attempt or evidence conflict exists. Resource absence does not supply this predicate.
...
The conservative absence branch uses existing typed evidence: a cleanup-step native readback with `availability=absent`, the exact `googleEditId`, `observation_surface=provider_response`, a sealed observation revision and the exact Google receipt. Use native matched_external only when every DELETE attempt for this effect is independently definitively not applied; its terminal mark remains FALSE when the observation proves resource state alone. A successful-delete provenance uses ordinary matched where readback is needed. Keep request_final separate in either case. The root’s native readback correlation and effective-evidence rules remain unchanged.
...
403/401, 429, a pre-dispatch denial and other definitive non-application outcomes prove no removal and cannot resolve cleanup. They follow existing authority/retry rules. A timeout followed by absence still leaves the DELETE uncertain unless its **request** independently becomes final. The expired_uncommitted_edit basis, local wall clock and generic GET/DELETE 404 cannot substitute for either predicate. If the decoder cannot distinguish confirmed invalidity from permission/transport failure, retain the explicit cleanup hold. A failed verification exit must never silently waive it.

This permits the important invalidated-edit path: the DELETE itself is conclusively rejected/non-applied **and** supported exact-resource readback proves the edit invalid/absent. It does not equate every rejection with cleanup.
...
Google documents invalidation of other open edits after console changes/commits and defines expiry as loss of validity. These facts explain the recovery requirement; they do not prove request finality or authorize generic-error shortcuts. Sources: [Edits guide](https://developers.google.com/android-publisher/edits), [AppEdit resource](https://developers.google.com/android-publisher/api-ref/rest/v3/edits), [DELETE contract](https://developers.google.com/android-publisher/api-ref/rest/v3/edits/delete). The current adapter's 204 branch is source behavior to validate under policy 2; the reference is not claimed to guarantee only that status.
...
1. No complete verification: final Play writes → confirmed inspection edit → invalidated edit/readback unavailable × N → durable exact cleanup → validated 204 terminal acknowledgement **or** final non-applied DELETE plus decoder-proven exact invalidity → retry_exhausted/readback_observation_budget → failed reopen with NULL evidence_revision_id, exact command scope and fresh-baseline prerequisite.
...
4. 403/429 DELETE with a still-valid edit never resolves cleanup. Timeout DELETE + exact absence readback still cannot reopen while request_final is missing. Any unresolved sibling write likewise prevents settlement/release.
...
New regression requirements include invalidated edit with no complete observation; mismatch then unreadable; later complete match superseding mismatch; generic DELETE 403/429/404 versus validated exact resource absence; ambiguous cleanup refusing reopen; accepted release grant→actual reschedule beyond window→same version accepted once under new context; A→B→A and no-op A→A; unchanged-schedule authority restoration; and commit-time erasure of the entire command/attempt cycle. These are required failing→passing fixtures, not executed tests.
