...
This appendix carries forward the accepted contracts that the v8 execution and Usage corrections do not replace. It describes the proposed design, not shipped behavior. v8 is authoritative for scheduling, inspection and binding, evidence/finality, retry and reconciliation, reopening, Usage, and privileged erasure. Earlier predicates for those areas must not be reinstated from this appendix.
...
If no applicable grant has remaining capacity, set `phase=blocked`, `hold_reason=retry_exhausted`, `next_run_at=NULL`. This hold means **inspection budget exhausted**, not an assumed provider failure. For this no-write/no-generation branch, the read model offers **Reconcile / Edit**, never mutation Retry or failed-write Reopen. Reconcile supplies new explicit bounded observation capacity while preserving the original approval for its still-unissued first write. Edit follows the normal zero-effect cancellation path, keeps ticket identity, and creates a revision needing fresh approval. Neither control implies a write occurred. A release event may resume only an actual awaiting_release hold; scheduling does not manufacture that hold to mint an event grant.
...
- inspection-only blocked/retry_exhausted after scheduling has NULL eligibility until an accepted reconcile supplies bounded inspection capacity. It then becomes ready with non-null due under the same approval. The read model offers Reconcile/Edit; it offers neither mutation Retry nor failed-write Reopen without their own prerequisites. There is no ready row with NULL next_run_at.
