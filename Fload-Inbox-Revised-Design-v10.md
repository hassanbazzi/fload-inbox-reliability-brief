...
This appendix carries forward the accepted contracts that the v10 execution and Usage corrections do not replace. It describes the proposed design, not shipped behavior. v10 is authoritative for scheduling, inspection and binding, evidence/finality, retry and reconciliation, reopening, Usage, and privileged erasure. Earlier predicates for those areas must not be reinstated from this appendix.
...
For native readbacks, `terminal_outcome=TRUE` means the adapter's pinned operation contract observed **terminal completion/non-application of the issued request**, using validated native request correlation or the closed Play edit-expiry rule. A GET's own response/request ID is not automatically the earlier write's request correlation. When only desired content was observed, use `terminal_outcome=FALSE`; finality may nevertheless already follow from the write's own terminal acknowledgement or definitive non-application.

`content_established(W, surface)` accepts a complete native `matched` or valid `matched_external` at the exact required surface without requiring `terminal_outcome=TRUE`. `request_final(W)` retains its independent terminal-acknowledgement/non-application/correlated-readback alternatives. Success requires both predicates, plus plan and unfinished-attempt rules. `handled_externally` additionally requires definitive non-application for every relevant Fload effect; `matched` never supplies that provenance.
...
`latest(scope)` is the latest eligible original observation attempt for that exact verification scope, resolved through its one late completion. Do not replace a newer attempt with an older late result. No observation is a real case, not a fabricated unreadable attempt.

### Readback transitions — exhaustive result handling

First persist and validate the native outcome, receipt and sealed observation under v8. Then recompute global finality; do not select a row below using only the just-observed subject's state.
...
| `unreadable/unavailable` or `unreadable/partial` | `verification_due`, non-null bounded-backoff next_run_at | `blocked/retry_exhausted`, readback_observation_budget, next_run_at=NULL |
| `unreadable/unsupported` | `blocked/unsupported_readback`, next_run_at=NULL; no automatic polling despite unused capacity | Same |
| No completed eligible readback yet | `verification_due`, non-null due time | `blocked/retry_exhausted`, next_run_at=NULL, derived qualifier `readback_observation_budget` |
...
Branch (c) requires phase=blocked, hold_reason=target_changed, and the latest eligible effective native readback for the exact blocked declared verification obligation to be mismatch, with a sealed complete observation, exact provider target, exact required surface and pinned comparison version. For after_effects use the approval’s required outcome scope; for before_successor use that step’s declared scope. The readback cannot be a prewrite/binding inspection, manual assertion, unrelated observation or an older result displaced by a newer eligible verification attempt. Preserve the exact failed scope after cleanup: its DELETE receipt cannot replace the listing mismatch. Required cleanup for a confirmed inspection resource must be resolved as specified below. A newer unfinished attempt, any non-final issued write or evidence_conflict refuses reopening; a later listing match cannot be ignored to reuse an older mismatch.
...
Under the command locks, record the selected observation in the command target's existing `evidence_revision_id`; settle the execution `failed`; release any guard it holds; clear current approval and open the same ticket. Preserve all acknowledged/applied effects, attempts and approval history. UI wording: “The requested state was not verified” with the recorded evidence; do not claim the provider rejected or rolled back an acknowledged write. A replacement requires a new sealed proposal and the existing qualifying **post-reopen** baseline-refresh protocol; no approval transfers.
...
### Play inspection cleanup after failed verification

**Play inspection cleanup after failed verification.** Only the already-compiled `play_inspection_edit_delete` may bypass the predecessor’s matched-content requirement. Require its exact declared predecessor `play_inspection_edit_create`, the same execution and approved revision, a validated successful creation receipt naming the exact inspection edit, and the latest complete native mismatch at that predecessor’s required scope after its observation capacity is exhausted. Every earlier issued write must be final, no attempt may be unfinished, and no evidence conflict may exist. Current approval, entitlement/policy, claim, canonical resource guard and immediate dispatch fencing remain required. The cleanup input remains the predecessor’s validated creation attempt/receipt, not the mismatching observation as a fabricated successful predecessor. The sole permitted request is DELETE of that exact inspection edit; no commit, listing-set, new edit or other successor is authorized by this exception.

When mismatch exhaustion is finalized or normalized, select this outstanding cleanup **before** persisting the terminal verification hold: atomically persist ready, next_step_id=the existing cleanup step, non-null due under its ordinary write policy, and a new schedule generation. Keep the immutable mismatch and the resource guard; no new provider attempt is invented. On the next delivery the same exact predicate selects cleanup despite the earlier failed content verification. This identical exception must exist in Core selection and the SQL predecessor guard; all other predecessors retain the matched-evidence rule. A crash recovers through this durable due state and ordinary claim admission, with no new queue or table.

Definitive cleanup acknowledgement satisfying its pinned request-finality contract resolves the cleanup; then persist blocked/target_changed with NULL due using the still-latest listing mismatch. Do not record content success or repeat the listing write. An ambiguous cleanup write follows global uncertain-write recovery and prevents settlement/reopen/guard release until its request is final. Definitive non-application follows the existing cleanup retry/disposition budget; current-authority failure blocks before admission. None may be bypassed merely to make Reopen available. A cleanup receipt or later readback can prove completion only under the existing exact-target and native-finality rules, not from elapsed expiry time or a generic missing-resource response alone.

**When unissued cleanup may remain unissued.** Failed settlement may abandon later ordinary plan effects. It may also leave inspection cleanup unissued when the inspection resource was never created: its creation step was unattempted or all creation attempts are definitively non-applied, with no successful creation receipt or unresolved request. Once this execution has a confirmed inspection edit, cleanup is a prerequisite to this failure settlement; it cannot be silently abandoned and cannot be sent after terminal settlement. Do not extend expired_uncommitted_edit to manufacture that prerequisite. A future separately validated native proof that an inspection edit was removed/expired would need its own exact-resource evidence contract; it is not asserted by this correction.

After confirmed inspection deletion, no new listing-content readback may be admitted through that deleted temporary edit. Preserve the last eligible pre-cleanup content evidence and offer Reopen; fresh current listing discovery belongs to the post-reopen baseline protocol. Cleanup-only receipts/observations do not displace the failed content scope. Reconcile while cleanup is unresolved remains limited to its exact supported recovery contract.

The exception adds no cleanup queue/table and applies only to the precompiled exact inspection DELETE. Other before_successor dependencies retain their matched-evidence rule.
...
Controls follow the qualifier and the existing command preconditions: observation exhaustion offers Reconcile and, only when zero write/generation attempts exist, Edit; it never invents mutation Retry or failed-write Reopen. A partial package may use only its already-defined safe retry/reopen branches. Permanent write rejection may use reopen branch (a) only after all issued writes are final. The target_changed branch-(c) transition above remains distinct.
...
Readback initial capacity is keyed by (step_id, subject_attempt_id, initial), anchored at that original subject's finished_at. INSERT requires prior starts < max_observations and started_at within that fixed observation window. Reconcile capacity is keyed by the exact step/subject/grant_command_id, starts at command acceptance, and has the same pinned count/window limits. An accepted event grants one bounded recovery scope, not one start. Its count, window and backoff use the scoped step's immutable policy, starting at command acceptance. Remove the attempt-level partial UNIQUE (organization_id,grant_command_id) WHERE grant_kind='event'; it incorrectly prohibits a second start under that grant. Keep command-level resume_hold uniqueness by execution/event/provider version. An ordinary non-unique index on (organization_id,grant_command_id) supports counting starts. Binding events cannot grant prewrite; publication events pin an exact step and issued subject. All INSERT admissions serialize on the execution lock, validate the command scope, count every original start including failure/expiry/cancellation, and require the fixed window and count. The one-unfinished-attempt rule still applies. Finalization consumes no second start.

Close an event grant when its scoped effective evidence establishes success at the required surface, complete non-pending mismatch, definitive no_editable_release, definitive non-application, unsupported recovery, or execution cancellation by undo/revise/reject. Cancelling an inspection for schedule/unschedule consumes its start but does not independently close the event grant; its original count/window and the current date gate still apply. Grant closure is derived from those immutable scoped attempts/effective evidence; it is not another mutable record. Queue retries, late completions and duplicate events cannot reopen it or reset its start count/window. A closed/expired event cannot make another independent valid grant unusable.

Transient unavailable/partial results may retry with bounded backoff under that same event grant. Typed pending-publication evidence is an explicit exception to treating every complete mismatch as decisive: it may continue bounded readback under that grant while the required published surface is pending. It never fabricates finality. At count/window exhaustion, use the ordinary purpose-specific hold table. An unresolved issued write still dominates local content results. A later explicit reconcile or genuinely new provider-version event may grant new bounded observations; repeated delivery of the same accepted event cannot. If all permitted observations fail, the visible hold requires Reconcile rather than polling forever.
...
- blocked/retry_exhausted from inspection-only scheduling: NULL I/O due until accepted reconcile supplies bounded inspection capacity. Current controls use the required derived qualifier and zero-effect preconditions. It becomes ready/non-null due only with usable capacity; there is no ready row with NULL next_run_at.
...
Next step: review these applied corrections, then explicitly resume implementation.
