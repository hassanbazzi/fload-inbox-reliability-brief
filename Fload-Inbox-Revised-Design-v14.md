# FLO-1355 — revised design v14

18 September 2026 · Corrected design for independent review · Implementation paused

**This version applies the v13 review corrections directly.** It preserves the full cleanup-read allowance when the edit has already expired or expires inside the ordinary window, using a separate bounded tail only when needed. A future scheduled release wait is visibly Scheduled until its date gate; it does not ask for Re-check now early or pretend a provider grant exists. The v13 history, uncertainty and safe guard-release corrections remain. The accepted architecture remains the foundation. Permanent tickets, immutable revisions, exact approvals, stable children, typed provider boundaries and the existing Actions/BullMQ architecture remain the design. This is a documentation specification; it is not migrated DDL, a passing implementation or a deployment.
...
This appendix carries forward the accepted contracts that the v14 execution and Usage corrections do not replace. It describes the proposed design, not shipped behavior. v14 is authoritative for scheduling, inspection and binding, evidence/finality, retry and reconciliation, reopening, Usage, and privileged erasure. Earlier predicates for those areas must not be reinstated from this appendix.
...
For policy 2, the initial readback grant of an uncertain play_inspection_edit_delete protects a post-expiry read within its existing N-start limit. It does not turn the first read after expiry into the only remaining read. Resolve these immutable inputs:
...
Keep the proposed cleanup policy-2 profile N=6, O=168 hours, B=1 hour, G=24 hours, factor 2. This is an operation-specific contract, not shipped behavior or a universal provider policy. Admission requires finite E and exact edit/receipt identity; missing or conflicting historical expiry is an explicit unsupported-recovery condition, never a guessed TTL. This policy change cannot revive exhausted older-policy grants.

Define R=E+B and D=F+O once from those original facts. Their comparison chooses exactly one derived branch; no stored branch flag or new grant kind is needed. Neither a later GET expiry, a late original acknowledgement, a restored credential nor a worker wake can re-anchor them. SQL/Core compare exact integer epoch microseconds for F, E, R, D and DB_now; multiply integral policy milliseconds by 1000 with checked range arithmetic. The internal Core selector uses an explicit instant-in-microseconds type backed by bigint, preserving database precision rather than rounding through JavaScript Date. Existing wire dates remain typed timestamps; public display precision cannot choose the admission branch.

| Fixed condition | Permitted original initial starts | Fixed final admission deadline |
|---|---|---|
| R < D: expiry fits inside ordinary window | Up to N ordinary starts; only the Nth has the additional lower bound R | initial_end=D |
| R >= D: expiry reaches or exceeds ordinary deadline | At most N−1 ordinary starts before D; at most one tail start in [R,R+G) | initial_end=R+G |

**Inside-window branch (R < D).** Let n be the number of already-admitted original initial-grant readbacks. If n>=N, there is no remaining start. Otherwise compute candidate=max(DB_now,backoff_not_before), and only when n=N−1 set candidate=max(candidate,R). Admit only if candidate<D. The first N−1 starts remain ordinary starts even when they happen after R. No ordinary slots are forfeited merely because expiry passed; no separate tail interval exists. A transient first read when E<=F can therefore be followed by later ordinary reads. The first-read backoff remains F+B, so an old expiry never permits early dispatch.

**Extended branch (R >= D).** Set ordinary_end=D, tail_due=R and initial_end=R+G. At most N−1 ordinary starts occur before D under ordinary backoff. No initial read starts in [D,R). For an unconsumed tail, candidate=max(R,backoff_not_before,DB_now) must be strictly less than initial_end. At/after R there is at most one initial start; unused ordinary slots cannot become extra tail starts. Derive tail consumption from an original initial readback admitted at/after R. R=D uses this branch: ordinary admission at D is forbidden, but the reserved start can begin at D and has G of fixed grace. A delayed ordinary read never waives the tail's applicable backoff.

Both branches count every original failed, cancelled and claim-expired start. Serialize count, candidate selection and INSERT on the execution lock; retain the one-unfinished-attempt rule and all current authority/fence checks. No start occurs at/after its branch's initial_end. An admitted read can finish after that start deadline; expired claims and late evidence follow the existing original-readback path and do not refund or add a start. Earlier valid quiescence or acknowledgement resolves the subject and makes remaining reads unnecessary.

Use one SQL/Core next_observation_start_at(scope,DB_now) selector for capacity, I/O admission, due calculation and normalization. It returns the earliest permitted candidate for that exact grant, or NULL. In the inside branch it accounts for the Nth start's lower bound; in the extended branch a future tail remains capacity after ordinary slots or D are exhausted. Retain the fixed initial_end state-only wake if backoff leaves no possible start; do not make an immediate I/O loop or invent rolling grace. Current claims are recovered/finalized before another admission. Original initial reads and separately admitted Reconcile reads are counted only against their own scopes; a Reconcile never silently consumes the protected initial slot.

Table A keeps uncertain/uncertain_write with the next permitted I/O or necessary state-normalization wake while applicable recovery remains. The inside branch normalizes against D; the extended branch uses R+G and cannot lose its future tail at D. Count exhaustion normalizes in finalization rather than waiting for a later deadline. Once initial capacity is exhausted and no other applicable grant remains, use blocked/uncertain_write, NULL due, retained exclusion and visible bounded Reconcile. An independently usable grant keeps its own due and state.

A narrow interval when R is just below D is intentional fixed-window behavior. Worker delay or backoff may miss it; the deadline never slides and no post-expiry read or provider success is guaranteed. An exact-present response after expiry is still mismatch/FALSE. Unavailable, malformed, throttled or denied reads, the expiry clock and the final count never prove cleanup, non-application or resend authority. The existing strict exact-edit quiescence decoder remains required.

The provider exposes the edit's absolute expiry and an exact GET representation; these support scheduling and complete-present classification, not a guarantee of successful readback after expiry. Sources: [AppEdit resource](https://developers.google.com/android-publisher/api-ref/rest/v3/edits), [GET edit](https://developers.google.com/android-publisher/api-ref/rest/v3/edits/get).

Required cases: E<=F with first 503/429 then valid later ordinary recovery; E=F+30 minutes with multiple permissible starts after R; R inside D but after the usual Nth backoff, so only that Nth start waits; R one microsecond below/equal/above D; R>D with early exhaustion and no gap I/O; delayed ordinary completion preserves tail backoff; later GET cannot change E; missing/mismatched expiry anchor; original-read crash and late evidence; immediate count exhaustion; earlier resolution cancels obsolete delivery; concurrent Reconcile and wakes; post-expiry present/unreadable evidence remains uncertain. The documentation arithmetic trace reproduces v13's one-start defect and checks six v14 examples, but does not validate platform code, SQL parity or native decoding.
...
Policy-2 cleanup readback uses the exact inside-window/extended-tail profile and selection rules above; do not infer that contract from binding-inspection defaults. All non-write kinds set terminal_on_acknowledgement=false; “not applicable” is not a nullable stored value. Policy values are fixed by a closed recovery-policy version, not caller-selected knobs. Initial inspection policy remains six starts in 168 hours, backoff one hour doubling to a 24-hour cap; prewrite uses the same bounded policy unless a future explicitly versioned operation contract changes it.
...
If the calculated next backoff crosses the last eligible window end, schedule the normalization wake at that window end, not beyond it. For policy-2 cleanup readback, use its fixed selected branch: D and the Nth-start placement for R<D, or R+G and the protected tail for R>=D. Only the extended branch retains capacity beyond the ordinary boundary. Count exhaustion is normalized in the finishing transaction; deadline expiry is normalized by the existing worker scan. Preserve the constraints requiring non-null due times for ready, verification_due and uncertain.
...
6. Lose the cleanup response, crash before/after readback persistence, redeliver, and admit a second ticket for the same package after safe failed-verification hold restoration but before user reopen: no repeated DELETE or stale guard release; writes are closed and the app guard is released in that transaction. If the DELETE was never sent and the edit remains present, preserve all N ordinary starts when R<D, placing the Nth no earlier than R; only R>=D uses N−1 early starts and a reserved tail. An already-expired edit with a transient first read must admit a later ordinary read. Release requires actual quiescence evidence, not the expiry clock. A matching late acknowledgement/rejection refines history only. Delay an ordinary read across tail_due and prove the reserved start still respects its backoff; exhaust the initial tail while an explicit Reconcile remains valid and prove recovery keeps that separate capacity.
