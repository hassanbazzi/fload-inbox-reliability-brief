# FLO-1355 — revised design v15

18 September 2026 · Corrected design for independent review · Implementation paused

**This version applies the v14 review correction directly.** The final protected cleanup read has one fixed end, max(ordinary deadline, expiry-plus-backoff-plus-grace), removing the short admission interval just before the ordinary deadline. The first N−1 reads keep their ordinary deadline; one protected read completes the same N-start allowance. Scheduled presentation, truthful capacity, history, uncertainty and safe guard release remain as accepted in v14. The accepted architecture remains the foundation. Permanent tickets, immutable revisions, exact approvals, stable children, typed provider boundaries and the existing Actions/BullMQ architecture remain the design. This is a documentation specification; it is not migrated DDL, a passing implementation or a deployment.
...
- A concrete draft can be approved while waiting for an editable release. It stays the same approved ticket, shows the prerequisite, and resumes when eligible while automatic recovery remains. If those checks finish first and its date gate is open, the ticket visibly asks for Re-check now; a genuine future release wait shows Waiting until the scheduled date; a newly applicable provider event can still resume it under the fixed grant rules. Whole-package waiting keeps editing available until the first actual write/generation admission.
...
This appendix carries forward the accepted contracts that the v15 execution and Usage corrections do not replace. It describes the proposed design, not shipped behavior. v15 is authoritative for scheduling, inspection and binding, evidence/finality, retry and reconciliation, reopening, Usage, and privileged erasure. Earlier predicates for those areas must not be reinstated from this appendix.
...
* needs_decision: a standalone ticket in needs_approval/uncertain/failed/blocked, including an exhausted release/publication wait, with the automatic/scheduled-wait exceptions below; or a parent with its own decision obligation or a current-member child in such a state.
* in_progress: queued/working/verifying/waiting, excluding rows already in needs_decision. needs_attention is their union.
* A container whose children are complete/declined and which has no own obligation projects completed, regardless of its stored open decision. Automatic release/publication waits and genuinely future scheduled release waits are in_progress; an exhausted wait whose applicable date gate is open is needs_decision. A scheduled release wait does not claim automatic capacity or suppress another decision obligation. This changes inbox placement, not the retained approval or the recorded prerequisite.
...
Personal state remains `action_read(organization_id,user_id,action_id,seen_attention_version,force_unread,updated_at)`. Marking read never changes workflow state. Attention increases for meaningful revision, approval/Undo, reopen, reject/acknowledge, external resolution, supersession, replacement of declined content, assignment to another person, and deduplicated visible progress transitions. Snooze/unsnooze, schedule/unschedule, archive/restore, self-assignment, retry, reconcile, resume_hold and binding alone do not create unread churn. Entering waiting creates one visible attention change. Becoming actionable after automatic capacity ends, or after a future release gate opens without capacity, is another deduplicated visible transition even when the hold enum is unchanged. It creates one attention increment, never one per poll. Restoring automatic wait capacity records the narrow no-attention progress transition below. Schedule/unschedule changes workflow version only for every actor, with no owner-dependent exception.
...
### Waiting presentation, real capacity and one recorded attention change

Use one SQL/Core projection for release/publication waits, detail, rows, current-member parent aggregation, list/count, command availability and progress recording. Separate the factual capacity(scope,asOf) calculation from the closed derived presentation waitRecoveryMode=scheduled | automatic | recheck_required. Require this field on the strict waiting wire variant and forbid it elsewhere; scheduled requires the existing scheduledFor timestamp to be non-null and future. No persisted wait-mode column is added.

Real automatic capacity means an already-admitted valid unfinished recovery read or an existing admissible initial/event/Reconcile opportunity remains for the exact obligation. Include all applicable grant scopes, fixed counts/windows, authority, backoff and genuine date gates. A normalization-only wake, expired grant or possible future event is not capacity. A future schedule never renews a grant or promises an event will arrive.

| Derived mode | Exact qualification | Inbox / visible control | Required progress cause tuple |
|---|---|---|---|
| scheduled | Current approved execution blocked/awaiting_release; new_work_pending=true; finite scheduled_for>asOf; no independent stronger blocker | in_progress; Waiting until [date]; no Re-check now | All three NULL |
| automatic | No applicable scheduled presentation; real automatic capacity remains | in_progress; bounded checking/waiting | All three NULL |
| recheck_required | No applicable scheduled presentation or real capacity; retain exact waiting hold | needs_decision; Re-check now | Exact binding/readback budget reason, step and subject below |

new_work_pending keeps its exact definition: no write or generation attempt admitted on this execution, including finished attempts. The scheduled exception suppresses only this binding wait's exhausted-capacity decision. Do not apply it to awaiting_publication, verification/recovery of an issued effect, uncertain writes, permission/credential/authority blockers, evidence conflicts, failed work or another decision obligation. Recompute such stronger facts first. A future date on a parent never postpones a child's independent execution. A parent still belongs to needs_decision when its own obligation or any current member independently needs a decision. Use the same precedence in the child-state CTE, root aggregation and counts; archive/snooze and signed cursor scope remain unchanged.

For due recheck_required rows show “Automatic release checks finished. Re-check readiness” or the publication equivalent, with Re-check now on the row. Preserve the exact approval and dated prerequisite facts. For scheduled show Waiting until [date] in its future-month group; edit, decline, schedule and Undo retain their normal admission rules. A newly submitted explicit Reconcile whose sole pending obligation is this parked binding scope returns existing invalid_transition before creating a grant. The canonical idempotency lookup still returns an already-recorded command outcome; replay is not revalidated against today's date. This refusal never blocks eligible recovery of an issued effect or a different obligation.

Reuse action_command.progress_exhaustion_reason, progress_step_id and progress_subject_attempt_id. On record_progress for blocked/awaiting_release with recheck_required require (binding_observation_budget, exact binding step, NULL). For blocked/awaiting_publication with recheck_required require (readback_observation_budget, exact verification step, original-write subject). Scheduled release and both automatic waits require all three NULL. These fields snapshot an actionable exhausted-wait explanation, not every grant's internal capacity state. Immutable attempts and grants preserve the underlying exhaustion facts even while presentation is scheduled.

Deduplicate phase/result/hold AND the exact nullable cause/step/subject tuple against the latest progress record for this execution/action ordered by its target result_version. Derive idempotency from that prior immutable progress identity and the new closed signature. A new required-recheck snapshot increments workflow and attention versions once, including the valid current-membership parent through existing guards. Do not change any personal action_read. Unchanged polls, duplicate syncs/commands and repeated normalization append nothing. scheduled and automatic share a NULL cause; moving between them alone creates no new progress record. The accepted schedule command and exact before/after date targets already record a scheduling change.

Before admitting a recovery grant, lock the current action/execution and compare the recorded snapshot with current actionable presentation at one captured DB time. If the date gate is open and actionable exhaustion is unrecorded, materialize its ordinary attention-bearing progress first, then admit the grant and record restoration through the existing CAS/target chain. A grant that beats the worker cannot erase that cycle. Restoration appends a separate NULL-cause progress record, keeps the same waiting hold and bounded due, increments workflow version and preserves attention. It follows a recorded required-recheck snapshot and the exact newly admitted granting command for this scope. Parent targets follow the validated child; they need not have the child's grant as their own prior command. No earlier grant is reset.

Keep guard_progress_attention's three exhaustive branches: validated grant-caused waiting restoration has attention delta 0; validated schedule-caused same-wait presentation/cause change has delta 0; every other newly recorded meaningful transition has delta 1. Every new progress record has workflow delta 1 and exact command targets with protected fields unchanged. Both silent branches need independent same-transaction admission proof from the locked canonical entry point and immediate SQL guard; preceding accepted cause-command result_version=progress previous_version supplies history attribution, not proof by itself. Historical adjacency, timestamps and a user-settable session variable cannot authorize silence. There is no caller-chosen attention delta.

Schedule/unschedule preserves attention for every actor. For the same waiting hold and exact obligation, recompute presentation and its required cause tuple in the scheduling transaction; do not require capacity itself to change. An already-exhausted wait moved into the future becomes scheduled even though capacity stays absent: append the attention-neutral NULL-cause progress when the prior cause was non-NULL. If it was already NULL, record only the date command and retain the future wake. Before scheduling, materialize any already-actionable unrecorded exhaustion under the old date/locks; scheduling cannot conceal it. Unscheduling can immediately expose exhaustion, but that command-caused tuple change is still attention-neutral. A changed hold or independent blocker uses its own transition rules. A no-op schedule cannot create capacity or a new event context.

### A scheduled release wait always has a durable gate wake

An exhausted scheduled wait must not commit NULL next_run_at. Persist its current scheduled_for as a state-only normalization wake through the existing due field/delivery generation and indexed blocked/awaiting_release scan; an earlier genuinely needed normalization or claim-recovery deadline may win first. No gate wake authorizes provider I/O. After processing an earlier expired-grant deadline while still scheduled, retain the future gate; a correctly normalized expired grant is not a new inconsistency on every poll. Do not keep choosing that same past deadline. Rescheduling advances the existing generation, and stale deliveries reload the current date before acting.

At/after the current gate, the worker locks and recomputes current authority, exact prerequisite/scope and real capacity. If valid capacity exists, normalize to automatic and its bounded I/O due; NULL cause remains NULL, with no gate-induced unread change. If no capacity exists and no stronger blocker applies, normalize to recheck_required, persist the exact binding cause, clear due and append one attention-bearing progress transition. If another blocker applies, use that blocker's truthful state and history. A provider offer is never assumed: if no sync succeeds, the durable gate still exposes Re-check now. A later applicable event can restore bounded capacity with the normal silent restoration; it cannot rewrite the gate history. If it arrives shortly after the gate exposed Re-check now, the existing unread update may remain even though checking has resumed. That accepted ordering records a real actionable interval; do not rewind attention or personal watermarks to hide it.

Make wait presentation/history/due atomic at commit. A scheduled release wait requires the correct NULL-cause snapshot plus a discoverable gate-or-earlier necessary normalization due. A due exhausted wait can commit NULL due only with its matching required-recheck snapshot and validated target chain. Later unrelated command versions need not equal that historical progress result_version. Repair or explicitly quarantine pre-existing violations before migration activation; activation assertions reject eligible missing-snapshot or scheduled-without-wake rows. No GET repairs them and no unindexed imaginary wake is assumed.

Read projections can show due required-recheck work before the worker records its attention transition; read APIs remain side-effect free. The worker compares the prior immutable snapshot rather than recomputing its old mode with today's clock. List and count use the same scoped SQL relation, one database asOf and one SQL statement or repeatable-read snapshot; a shared clock alone does not cure separate READ COMMITTED snapshots, and cursors do not freeze later requests.

A personal mark-read while scheduled acknowledges only the current attention version. Gate passage without capacity creates new attention, so the item becomes unread once. A read after the date but before worker normalization can likewise precede that durable increment, as with existing deadline normalization. force_unread and other users' watermarks remain untouched. Retain the conservative actor watermark CAS: a Re-check transaction that discovers unseen system progress does not prove the actor saw it. A later explicit mark-read can acknowledge the presented version; no blanket acknowledgement of all transaction increments is added.

Required fixtures: exhausted September wait→December scheduled with no early button/needs_decision/bump; new parked Reconcile refused without a grant while old command replay works; no sync at the gate→one required-recheck/unread transition; valid capacity at the gate→automatic without a false bump; earlier grant deadline retains gate wake and does not busy-loop; reschedule A→B→A, no-op, unschedule and stale delivery; unrecorded exhaustion before scheduling or gate-time grant recorded first; exact equality at gate; parent with parked child plus due blocked child counted once in needs_decision; no suppression of permission/conflict/publication/uncertain-effect recovery; personal mark-read races and force_unread; runtime and migration rejection of missing snapshot/wake. Existing M6 fixtures for repeated restore/exhaust cycles, exact command guards, duplicate normalization and list/count agreement remain mandatory. These are implementation requirements, not executed runtime tests.
...
| `unreadable/no_editable_release` | `blocked/awaiting_release`; use the shared waiting/due projection: bounded next_run_at while applicable capacity remains, a state-only future schedule gate when scheduled without capacity, otherwise NULL with required-recheck history. Exhaustion does not turn this readiness fact into unsupported_readback. |
| `unreadable/unavailable` or `unreadable/partial` | If the last authoritative binding readiness fact was no_editable_release and no later complete binding superseded it, retain awaiting_release with the same shared capacity/future-gate/required-recheck due rule; display the failed latest check and date of the last readiness fact. Otherwise ready with bounded due while capacity remains; on exhaustion blocked/retry_exhausted with binding_observation_budget qualifier and NULL due. Do not invent a no-release fact. |
...
`capacity(scope, now)` means an existing admitted grant for that exact step/purpose/subject has a remaining feasible start under its immutable count/window and authority rules, including the reserved future initial cleanup slot below. next_observation_start_at distinguishes schedulable capacity from admissible-now I/O; a future reservation does not authorize an early call. An event has one fixed bounded grant using its step policy from command acceptance; it does not reset initial/reconcile counts. A closed event grant cannot be reused even if its numeric count/window would otherwise allow another start. Finalization consumes no second start. A late completion updates effective evidence but consumes no grant.
...
### Release the app guard before waiting for failed-verification reopen

When required Play inspection cleanup resolves and the exhausted content-verification hold is restored (blocked/target_changed or blocked/retry_exhausted with readback_observation_budget), the same transaction must release the execution's currently held resource guard. Require every issued write final, no unfinished attempt, no evidence conflict, all required cleanup resolved and the complete static plan's mutation phase finished. Record writes_closed_at using the existing monotonic field and clear active mutation scheduling before release. The ticket retains its approval, execution, failed verification evidence and reopen option; it does not retain app-wide exclusion while waiting for a human.

Only release the guard if its holder and acquisition generation still belong to this execution. A replay is harmless; a late finalizer cannot release another ticket's later acquisition. The writes-closed admission guard forbids any further mutation on this execution. Any remaining allowed diagnostic/content verification is read-only, uses existing bounded authority, and cannot read through a deleted/invalid inspection edit. A later correction requires reopen, a fresh baseline, a new revision/approval/execution and normal resource acquisition.

The same rule may apply to an exhausted after-effects verification hold only when that plan's mutation phase is already complete and every cleanup obligation is resolved. It is not a blanket release on blocked, not release around an unresolved sibling effect, and not early writes closure on an incomplete dynamic package. The existing prewrite-release exception still leaves writes_closed_at unchanged. Add the explicit pre-reopen regression: E1 enters the safe failed-verification hold, E2 acquires the same app guard while E1 remains unreopened, and E1's late evidence/replay cannot touch E2's generation.
...
### Present inspection edit and the reserved after-expiry recovery read

An uncertain cleanup DELETE may never have reached Google. A complete successful GET of its exact edit is therefore a meaningful cleanup result: HTTP 200 with a valid AppEdit object and exact ID yields native mismatch, terminal_outcome=FALSE, availability=present, exact googleEditId and provider_response surface. Require the pinned cleanup fingerprint/version and same-row Play receipt (HTTP status, exact edit ID and returned well-formed finite expiry). Listing text fields remain unspecified: this observation proves only edit presence. Preserve the observed expiry in that read's receipt; it does not move the original deadline anchor. A returned edit does not become quiescent just because its expiry is in the past. Wrong scope/ID, malformed data, denial and throttling remain the appropriate closed unreadable outcome.

Presence is not non-application of the DELETE and does not permit a resend. While the original DELETE remains non-final, table A dominates: “Outcome uncertain · inspection edit still present.” Do not convert this cleanup mismatch into listing target_changed, failed verification evidence or a content-edit approval requirement.

For policy 2, the initial readback grant of an uncertain play_inspection_edit_delete protects one post-expiry read within its existing N-start limit. An already-expired edit retains ordinary retries; a near-deadline expiry does not squeeze the protected read into seconds. No additional stored field, grant kind or second budget is needed.

| Symbol | Existing authoritative fact |
|---|---|
| F | Original DELETE's immutable finished_at after uncertain/claim-expiry finalization |
| E | play_edit_expires_at from the exact inspection-create input receipt resolved and validated before this DELETE's admission |
| N | Cleanup step max_observations |
| O | Cleanup step observation_window_ms |
| B | Cleanup step backoff_initial_ms |
| G | Cleanup step backoff_max_ms |

Keep the proposed cleanup policy-2 profile N=6, O=168 hours, B=1 hour, G=24 hours, factor 2. This is an operation-specific proposed contract, not shipped behavior or a universal provider policy. Admission requires finite E and exact edit/receipt identity; missing or conflicting historical expiry is an explicit unsupported-recovery condition, never a guessed TTL. The policy cannot revive exhausted older-policy grants.

Compute these absolute times once from the original facts:

```text
R = E + B
D = F + O
H = max(D, R + G)       // initial_end, not a stored new deadline
```

A later GET expiry, late original acknowledgement, restored credential or worker wake cannot move any anchor. SQL/Core calculate using exact integer epoch microseconds; checked integral policy milliseconds are multiplied by 1000. The internal Core selector uses a strict instant-in-microseconds type backed by bigint, preserving database precision rather than rounding through JavaScript Date. Existing wire dates remain typed timestamps; display precision cannot affect admission.

| Start allowance | Earliest permissible start | Exclusive deadline |
|---|---|---|
| Up to N−1 ordinary original initial readbacks | max(DB_now, applicable backoff_not_before) | D |
| At most one protected original initial readback | max(DB_now, applicable backoff_not_before, R) | H=max(D,R+G) |

**Ordinary starts have priority.** Let n be the count of already-admitted original initial-grant readbacks for this exact step/original-write subject. First determine whether the protected allowance has been spent: protected_consumed is true when n>=N OR any such original initial readback has admitted started_at>=D. Late-evidence rows and reads belonging to another grant are excluded. Before D, the Nth admitted original consumes the protected allowance; at/after D, any admitted original initial readback consumes it even if fewer than N−1 ordinary reads ran. Both are derived from immutable admissions, not a new attempt flag.

After existing claim/finality/authority checks, use this one selector under the execution lock:

```text
if protected_consumed:
    no remaining initial start
else:
    ordinary_candidate = max(DB_now, backoff_not_before)
    if n < N - 1 and ordinary_candidate < D:
        return ordinary_candidate
    protected_candidate = max(DB_now, backoff_not_before, R)
    return protected_candidate only if protected_candidate < H
```

Actual INSERT rechecks that selector at the captured admission time under the same lock, and records that same DB instant as the original started_at. A queued ordinary candidate from before D must qualify again at delivery; it can use only the protected allowance if now at/after D. A future returned candidate is a due time, not permission to perform I/O early. With an unfinished attempt, finalization or claim-expiry recovery precedes selection/admission of another; do not infer its eventual backoff from a queue timer.

Crossing R alone never consumes the protected allowance: the first N−1 starts can still be ordinary before D, including multiple starts after expiry. The initial backoff remains F+B. When no ordinary start can fit before D, one protected start remains eligible, subject to R/backoff/H, even if only zero or a few ordinary reads ran. The missing ordinary slots are then forfeited; they cannot become extra protected retries. If R>D, no initial read can start in [D,R). R=D needs no special branch. Exact H is excluded, and grace is never recalculated relative to delivery time.

Count every original failed, cancelled and claim-expired start. Preserve the one-unfinished-attempt rule and current fence/authority checks. A protected start before D consumes the allowance just as one after D does. Once it finalizes without resolving the subject, normalize spent initial capacity immediately even if n<N and H is still future; do not wait for a seventh read, unused ordinary slots or H. Valid late completion belongs to its original readback and consumes no extra start. A read admitted before H may finish later through the existing live-claim/late-evidence rules. Earlier valid quiescence or acknowledgement makes remaining opportunities unnecessary.

Use the same SQL/Core next_observation_start_at(scope,DB_now) for capacity, admission, due calculation and normalization. D is only the ordinary cutoff; it must not erase an unspent protected opportunity before H. If applicable backoff already makes the protected candidate >=H, retain the fixed H state-only normalization wake under the existing rule, without performing I/O outside the interval. Current claims are handled first. Count consumption normalizes in finalization; deadline expiry is recovered by the existing due scan. If H has already passed and no live claim takes precedence, normalize now under the locks instead of repeatedly rescheduling an already-past H. No API timer or new queue authority is introduced.

Original initial reads and separately admitted Reconcile reads count only against their own scopes. A Reconcile read cannot spend the protected initial allowance, and exhausted initial capacity cannot discard another usable grant. Table A retains uncertain/uncertain_write with the next permitted I/O or necessary state-normalization wake while recovery remains. When initial capacity is spent/expired and no other applicable grant remains, use blocked/uncertain_write, NULL due, retained exclusion and visible bounded Reconcile.

The fixed allowance has a continuous endpoint around R=D: R=D−1 microsecond gives H=D+G−1 microsecond; equality gives H=D+G. Backoff or a long-lived claim can still consume the available interval, and provider/worker availability is not guaranteed. A clock, last allowed read or exhausted count never proves cleanup, non-application or resend authority. An exact-present response after expiry remains mismatch/FALSE; unavailable, malformed, denied or throttled evidence cannot become quiescence.

Google exposes the exact edit expiry and GET representation; these support scheduling and complete-present classification, not guaranteed cleanup or a universal lifetime. The strict exact-edit quiescence decoder remains required. Sources: [AppEdit resource](https://developers.google.com/android-publisher/api-ref/rest/v3/edits), [GET edit](https://developers.google.com/android-publisher/api-ref/rest/v3/edits/get).

Required fixtures: E=F+166h with five ordinary starts and the protected read delayed beyond D but before H; R=D−1 microsecond, D, D+1 microsecond and exact H exclusion; old expiry with transient first read then later ordinary recovery; only zero/two ordinary reads before D followed by one protected read and immediate exhaustion despite n<N; protected admission before D then failure/crash without refund; delayed ordinary completion respecting backoff; R>D with no gap I/O; concurrent wakes/INSERTs; original late evidence; independent Reconcile accounting; obsolete delivery after earlier resolution; changed GET expiry cannot re-anchor; missing/wrong receipt; post-expiry present/unreadable remains uncertain. The documentation arithmetic checker reproduces the old boundary failure and verifies the timing/count rules only; SQL parity, native decoding, claims and concurrent runtime behavior remain implementation tests.
...
The I/O due lower bound is the greatest of command admission, original Undo deadline, new schedule if any, and the recorded policy backoff. For obligations outside the same-wait scheduling exception, persist the earlier required deadline-normalization wake when that lower bound falls outside the existing grant window; that wake changes state only and cannot bypass the date to perform I/O. For a qualifying unchanged release wait whose schedule change alters presentation or its cause tuple, normalize that change immediately with the attention-neutral rules above, even if capacity stays absent. Retain the current date gate as a state-only wake; do not defer this command-caused mode change until a grant deadline. A publication or issued-effect recovery obligation is never postponed by its historical schedule. Normalize any pre-existing unrecorded exhaustion before applying that exception. Do not reuse an old claim expiry as backoff. The no-effect exhausted case offers Reconcile/Edit; event recovery additionally follows the exact binding scope rule below. No new grant or approval is created by scheduling.
...
|---|---|---|
| progress_step_id | text NULL | Required for exhausted record_progress (retry_exhausted or recheck_required waiting), every resume_hold and accepted unverified branch-(c) reopen; forbidden otherwise |
| progress_subject_attempt_id | text NULL | Required for readback/effect exhaustion qualifiers (including recheck_required publication waits), publication_detected and unverified branch-(c) reopen; NULL for binding/prewrite qualifiers and release_detected; forbidden otherwise |
| progress_exhaustion_reason | actions.action_exhaustion_reason NULL | Required for retry_exhausted record_progress; binding_observation_budget/readback_observation_budget respectively on recheck_required release/publication waiting progress; exactly readback_observation_budget on accepted unverified branch-(c) reopen; forbidden otherwise |
...
- record_progress: accepted system/worker; progress_execution_id and progress_phase required; phase restricted to verification_due/uncertain/blocked/settled; result present exactly for settled; hold present exactly for blocked/uncertain; all resume fields NULL. The scope/reason fields follow the exact retry_exhausted and recheck_required waiting matrix above; scheduled release progress, automatic waiting progress and every other hold/result require all three NULL. The narrowly validated automatic-capacity-restored waiting transition and the validated schedule-caused same-wait presentation/cause transition both record history/version without attention; every other newly recorded meaningful progress transition increments attention once. Each exception requires its exact cause, scope and same-transaction admission proof described above.
...
| Readback mismatch: exact inspection edit present | required successful GET receipt, exact edit ID and finite returned expiry; sealed complete present edit; terminal_outcome=FALSE; listing fields unspecified |
...
Policy-2 cleanup readback uses the exact ordinary/protected profile and single selector above; do not infer that contract from binding-inspection defaults. All non-write kinds set terminal_on_acknowledgement=false; “not applicable” is not a nullable stored value. Policy values are fixed by a closed recovery-policy version, not caller-selected knobs. Initial inspection policy remains six starts in 168 hours, backoff one hour doubling to a 24-hour cap; prewrite uses the same bounded policy unless a future explicitly versioned operation contract changes it.
...
- awaiting_release: only a recorded complete no-release readiness fact supports the hold. A bounded fallback or applicable event/reconcile grant may admit binding inspection under current authority. Apply schedule while new_work_pending; exhaustion retains awaiting_release with a state-only future gate wake when scheduled, otherwise NULL due and recorded required-recheck progress;
...
Define a read-only `normalization_due_at(E,now)` from existing execution/step/grant/attempt facts. It is due immediately when persisted active/waiting presentation, cause history or required due is inconsistent with the rules above, including a due exhaustion or a passed scheduled release gate. A still-live grant deadline can be an earlier state-only wake; once expired capacity is correctly normalized while scheduled, the next wake is the current gate, not that same past deadline. The gate remains independently discoverable even with zero grants. For multiple existing applicable grants, normalize against their combined remaining eligibility: expiry of one does not block an independently valid explicit grant.
...
A due execution already in its correct required-recheck hold with NULL due and matching latest immutable progress snapshot has no wake until new evidence/grant/state arrives. A future scheduled release wait instead requires a discoverable gate-or-earlier necessary normalization due and its NULL-cause snapshot, even with zero capacity. Make their atomic persistence a commit-time invariant: a due recheck_required awaiting_release/awaiting_publication row cannot commit NULL next_run_at unless the latest progress snapshot matches its exact required-recheck scope, cause and resulting execution-ticket target version, with the required current parent targets. Validate that snapshot's immutable target chain; later unrelated command versions need not equal its recorded result_version. Finalization, grant admission, schedule admission and normalizer paths must materialize any missing snapshot under the same locks before clearing the due; a still-pending normalization remains discoverable through a non-null allowed due. Migration/backfill must repair or explicitly quarantine pre-existing violations before enabling this guard and activating the due scan; activation assertions must reject unresolved eligible NULL-due/missing-snapshot rows and scheduled release waits missing their gate wake. GET does not repair them, and no unindexed or imaginary wake is relied upon. This does not add a queue engine or API timer.

On this wake, acquire the normal locks and **apply the tables without admitting I/O**, without inventing an observation and without resetting a count/window. Persist the resulting hold/phase and correct due: keep the current gate-or-earlier required normalization wake for a scheduled release wait, retain bounded I/O due for usable capacity, and clear due only for a correctly recorded required-recheck or other NULL-due hold. Clear any no-longer-live claim through existing recovery rules and advance delivery generation for the state/due change. Record progress only when its phase/result/hold/cause signature changes; an unchanged hold can still have a new required-recheck cause, but scheduled↔automatic with the same NULL tuple creates no extra progress record. Apply the exact scope/attention guards above. Unchanged polls do not change history, versions or attention. I/O eligibility can be NULL while the durable scheduled normalization wake remains non-null. The queue never needs an impossible final readback merely to reveal exhaustion.

If the calculated next backoff crosses the last eligible window end, schedule the normalization wake at that window end, not beyond it. For policy-2 cleanup readback, D ends ordinary admission while H=max(D,R+G) is the protected start deadline. Preserve an unspent protected opportunity across D; normalize protected consumption immediately after finalization even if fewer than N starts exist, and never re-anchor H. Count exhaustion is normalized in the finishing transaction; deadline expiry is normalized by the existing worker scan. Preserve the constraints requiring non-null due times for ready, verification_due and uncertain.
...
If inspection is in flight, finish it under the command fence before clearing the claim and changing delivery generation. Its late result cannot dispatch. An unsuperseded complete no-release fact recomputes blocked/awaiting_release with bounded I/O due when capacity remains, a state-only December gate while future scheduled, or NULL only for due required-recheck with its recorded exhaustion snapshot. Scheduling an exhausted wait clears its cause through the neutral progress rule; the December wake records a new required-recheck cause once if no capacity exists then. Without that fact, usable capacity yields ready/non-null due and exhaustion yields blocked/retry_exhausted/NULL due with the inspection qualifier. No pre-claim hold is copied; immutable evidence governs both claimed and unclaimed cases. Schedule changes version but not attention or personal reads. If any write/generation already exists, schedule refuses execution_started; existing-effect verification remains due independently of the date. Scheduling a parent alone never silently moves child executions.
...
6. Lose the cleanup response, crash before/after readback persistence, redeliver, and admit a second ticket for the same package after safe failed-verification hold restoration but before user reopen: no repeated DELETE or stale guard release; writes are closed and the app guard is released in that transaction. If the DELETE was never sent and the edit remains present, allow up to N−1 ordinary starts before D and one protected start no earlier than R/backoff and strictly before H=max(D,R+G). Crossing R does not forfeit ordinary starts before D, and missing ordinary slots cannot become multiple protected reads after D. An already-expired edit with a transient first read must admit a later ordinary read. Release requires actual quiescence evidence, not the expiry clock. A matching late acknowledgement/rejection refines history only. Delay the protected read beyond D but before H and prove it remains admissible with backoff satisfied; admit it after fewer than N−1 ordinary starts and prove its finalization consumes the remaining initial allowance immediately; retain independently valid Reconcile capacity throughout.
...
8. Release V seen editable → grant accepted → complete no_editable_release closes grant → V editable again under the same context: no duplicate event grant; with no other automatic capacity and an open date gate the row moves to needs_decision and receives one attention increment; a future scheduled release wait instead preserves its gate wake and Scheduled presentation. Re-check now supplies one separate bounded Reconcile grant and records automatic restoration without another unread bump. Duplicate syncs, duplicate Reconcile keys and no-op schedule changes do not renew capacity.
...
- M5: exact-present cleanup evidence is mismatch/FALSE, not unavailable or proof of non-application. The initial cleanup-readback budget reserves one automatic opportunity after the provider-stated edit expiry, with fixed bounded grace and no renewal. Time alone never proves cleanup.
- M6: exhausted release/publication waits become needs_decision with one deduplicated attention change. The existing reason/step/subject fields preserve that transition; capacity restoration records a no-attention progress snapshot so repeated cycles remain correct. One classifier governs list/count/parent/detail semantics.
- L7: after all writes are final and required cleanup resolves, the exhausted verification hold closes further mutations and releases the app guard in the same transaction, before user reopen.
- Added exact-tail count/deadline/admission, provider-present, missed-window, duplicate-normalization, repeated-wait-cycle, parent/list/count/read-state and pre-reopen guard-release fixtures as implementation requirements.

V13 adds no stored columns or relations. It extends exact predicates, the existing progress-field matrix and attention guard; new projection values remain closed types. All native fixtures, migrations and runtime failure tests remain required before implementation can be called complete.

## What changed from v13 to v14

- M7: cleanup readback keeps the full ordinary N-start allowance when expiry falls inside its fixed window. Only expiry at/beyond that deadline uses the separate bounded tail. Exact equality, backoff, counts, immutable anchors and independent grants have explicit rules.
- L8: future scheduled release waits show Scheduled / Waiting until the date and suppress early Re-check now. This is derived presentation, never fictitious recovery capacity; the existing worker handles the durable gate wake.
- Retain the conservative personal-read watermark CAS. A command that discovers unseen system progress does not prove the actor saw that progress; no implicit acknowledgement of an entire transaction is added.
- No stored fields or relations are added. The closed waiting wire union, SQL/Core projections, progress rules and acceptance cases must change together. Platform implementation remains paused.

## What changed from v14 to v15

- L9: replace the two cleanup timing branches with up to N−1 ordinary starts before D and one protected start before H=max(D,R+G), with lower bound R and unchanged backoff. The endpoint no longer jumps when R crosses D.
- Derive protected consumption from original admissions: n>=N or an original initial start at/after D. An after-D protected read is permitted even after fewer ordinary reads, then ends initial capacity immediately on finalization. No stored flag, extra start or new grant is added.
- Align the general grant contract, policy description, normalizer and case 6 with the single selector. Keep the accepted Scheduled mode, gate wake, attention history and actor watermark policy unchanged.
- Record the accepted consequence that a gate can make work unread before a later legitimate event restores automatic capacity. This is truthful history; automatic recovery never rewinds attention or another user's read state.

No stored fields, enum values or relations are added in v15. All application, migration, native-provider and concurrency proof remains required; implementation is paused.

Next step: independent review of the applied boundary correction, then explicitly resume implementation.
