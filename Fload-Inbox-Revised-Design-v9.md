...
This appendix carries forward the accepted contracts that the v9 execution and Usage corrections do not replace. It describes the proposed design, not shipped behavior. v9 is authoritative for scheduling, inspection and binding, evidence/finality, retry and reconciliation, reopening, Usage, and privileged erasure. Earlier predicates for those areas must not be reinstated from this appendix.
...
An applicable `release_detected` event may grant **one** binding inspection after timer exhaustion **only** for an actual awaiting_release hold justified by the recorded readiness fact, for the same execution/approved target, with the stable provider-version identity and current canonical authority. Enforce unique event admission and recheck exact approval, policy, entitlement, Undo and schedule at inspection admission. Ignored/inapplicable events create no grant. A revoked decision, unrelated release or an event on unsupported_readback cannot bypass those checks. The event conveys a reason to inspect, not a binding or approval.
...
| `unreadable/unavailable` or `unreadable/partial` | Ready with bounded observation backoff while this exact grant has capacity; otherwise blocked/unsupported_readback, NULL due. No write. |
...
4. For undo/edit/reject, set execution cancelled and clear the claim. For schedule/unschedule, retain approval/execution and choose the exact capacity-dependent result defined below: ready with a non-null due when an existing inspection grant has capacity; otherwise blocked/retry_exhausted with NULL due. Retain a stored permitted hold only when it was unclaimed at command admission. Clear claim/next-step claim, advance delivery generation and apply the ticket change. Release inspection-only exclusion through prewrite_release_allowed.
...
| Complete `mismatch` without that typed pending-publication proof | `verification_due`, non-null bounded-backoff next_run_at | `blocked/target_changed`, next_run_at=NULL |
...
| `unreadable/unavailable` or `unreadable/partial` | `verification_due`, non-null bounded-backoff next_run_at | `blocked/unsupported_readback`, next_run_at=NULL |
...
`before_successor`: the matching required-surface verification is a dependency. Do not admit its successor until satisfied. A mismatch/unreadable hold blocks that successor even when the provider acknowledged the predecessor. Do not declare the whole execution complete solely because this dependency passed.
...
For exhausted complete after-effects mismatch, the transition into blocked/target_changed is the entry to reopen branch (c). The latest mismatch must still meet that branch's exact target/surface/evidence requirements. A before-successor mismatch does not silently acquire branch-(c) eligibility; it remains a blocked dependency unless another explicitly defined reopen branch applies.
...
Failed settlement through `reopen` has three branches. Branch (a) requires blocked/retry_exhausted and the failing step's latest write definitively known_not_applied/permanent. Branch (b) requires blocked/target_changed and a finalized complete mismatch prewrite naming the next required write step. Branch (c) is the after-effects mismatch path below. Shared preconditions for branches (a), (b) and (c): exact current action/version/revision/approval/execution pins; execution nonterminal; no unfinished attempt; **every issued write on this execution satisfies request_final**; no evidence conflict; canonical command authority. None of the branches grants a new write.

Branch (c) requires `phase=blocked`, `hold_reason=target_changed`, and the latest eligible effective native readback for the failed approval's required verification scope to be `mismatch`, with a sealed complete observation, exact provider target, exact required surface and pinned comparison version. It must be an after-effects readback of this execution, not a prewrite, manual assertion, unrelated observation or an old readback displaced by a later verification attempt. A newer unfinished attempt refuses reopening; a later readback/match cannot be ignored to reuse an older mismatch. The diagnostic `evidence_conflict` hold never qualifies.
...
Schedule/unschedule preserves the exact approval and execution and admits no new authorization. Under the existing command-fence protocol, it finishes the interrupted inspection as unreadable/cancelled, clears the claim and next-step claim, advances delivery generation and releases any inspection-only guard. An interrupted claim does not restore a supposed previous hold. Select the next required inspection from the immutable plan and current evidence, then evaluate an **existing applicable observation grant**, including all starts already consumed and its original window. The cancelled inspection consumed its start; cancellation refunds neither count nor time.

If a fresh inspection start remains authorized, set `phase=ready`, `hold_reason=NULL`, and a non-null due equal to the greatest of command admission time, original Undo deadline, the new schedule if any, and the policy backoff lower bound derived from that grant's recorded failed observations. No old claim expiry is reused as the backoff. The next delivery admits a new inspection; its result determines the next real hold or progress state.

If no applicable grant has remaining capacity, set `phase=blocked`, `hold_reason=retry_exhausted`, `next_run_at=NULL`. Derive the binding_observation_budget or prewrite_observation_budget qualifier from the pending inspection obligation, as required below; do not infer a provider failure. For this no-write/no-generation branch, the read model offers Reconcile / Edit, never mutation Retry or failed-write Reopen. Reconcile supplies explicit bounded observation capacity while preserving approval for the still-unissued first write. Edit uses zero-effect cancellation and creates a revision needing fresh approval on the same ticket. A release event may resume only an actual awaiting_release hold justified by recorded authoritative readiness evidence; scheduling does not manufacture that hold or an event grant.

When the execution was already blocked and unclaimed at command admission, its stored awaiting_release/resource_busy hold is available and may be retained. Recompute eligibility under the same budget rules without resetting its grant. Other diagnostic/permanent holds are not silently converted to ready. Any admitted write or generation, finished or unfinished, still causes `execution_started` refusal.

### Required current hold qualifier and honest historical explanation
...
- `prewrite_observation_budget`: a schedule/cancellation path stopped with no capacity for its pending prewrite.
- `readback_observation_budget`: verification became due after its window without a usable observation.
...
This is a derived current read-model discriminator, not a stored column. Include its exact step and, when applicable, subject attempt in the typed projection. Historical progress displays its actual immutable recorded facts. A historical qualifier is permitted only when existing immutable command-target/attempt identity anchors establish the causal obligation and a deterministic admitted-evidence cutoff; timestamps alone are insufficient because a late response may have been captured earlier than its admission. When those anchors are absent, show an explicit “Reason unavailable” diagnostic with the recorded hold, rather than reconstructing a cause from current attempts. This pass adds no historical field. Missing/contradictory current evidence is an invariant diagnostic, never a default permanent rejection, permission failure or new command authority.
...
The existing SQL due scan returns the earlier of the next permitted I/O due and this required state-normalization wake. A claimed execution uses claim expiry first. An execution already in the correct exhausted hold with NULL due has no normalization wake until new evidence/grant/state arrives. This does not add a queue engine or API timer.
...
If inspection is in flight, finish it under the command fence before clearing the claim and changing delivery generation. Its late result cannot dispatch. Remaining grant capacity yields ready/non-null due; exhausted capacity yields inspection-only blocked/retry_exhausted/NULL due with Reconcile/Edit. No prior hold is reconstructed. Schedule changes version but not attention or personal reads. If any write/generation already exists, schedule refuses execution_started; existing-effect verification remains due independently of the date. Scheduling a parent alone never silently moves child executions.
...
3. Before-successor mismatch prevents the successor; a matched dependency alone cannot settle the package. Incomplete App Clip plan remains incomplete.
...
6. Complete no-release discovery exhausts timers but retains awaiting_release; one valid event grants one fresh binding read; duplicate/unrelated/revoked events do not.
...
Next step: review these applied corrections, then explicitly resume implementation. No full implementation/test pass or production deployment is claimed.
