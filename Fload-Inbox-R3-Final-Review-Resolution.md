...
| D2 · New blocker admission | A refusal on never-admitted work preserves its typed planning reason; a new client-clearable card uses the same admission guard and consumes a slot. Exhausted headroom stops additional new-work preparation. Blockers on existing unfinished work always surface, even above five |
...
A namespaced transaction-scoped advisory guard is preferred over locking the shared asset row, avoiding unnecessary coupling to sync/metadata updates. Its encoding and global lock-order integration still require implementation proof. Proposed closed refusal `client_attention_capacity` reuses immutable caller-minted command receipts; replay stays refused and a deliberate new attempt needs a new key/fresh pins. Deterministic producer keys instead use the subsequent [locked deferral and durable re-offer contract](Fload-Inbox-R3-Parallel-Work-Coordination.md#deterministic-producer-keys-defer-before-command-creation-f2), without consuming the key for capacity alone. No open payload or mutable slot counter is added.
...
The [merged-fixes review](Fload-Inbox-R3-Merged-Fixes-Resolution.md) supersedes the source checkpoint, records PRs1445–1448 merged, distinguishes client-clearable from Fload-owned zero-slot cards and corrects deterministic-key capacity handling.
