...
This appendix carries forward the accepted contracts that the v13 execution and Usage corrections do not replace. It describes the proposed design, not shipped behavior. v13 is authoritative for scheduling, inspection and binding, evidence/finality, retry and reconciliation, reopening, Usage, and privileged erasure. Earlier predicates for those areas must not be reinstated from this appendix.
...
* needs_decision: a standalone ticket in needs_approval/uncertain/failed/blocked, including an exhausted release/publication wait, with the automatic-wait exception below; or a parent with its own decision obligation or a current-member child in such a state.
* in_progress: queued/working/verifying/waiting, excluding rows already in needs_decision. needs_attention is their union.
* A container whose children are complete/declined and which has no own obligation projects completed, regardless of its stored open decision. Waiting for an editable release or publication is in_progress only while automatic recovery remains; exhausted waits are needs_decision. This changes inbox placement, not the retained approval or the recorded prerequisite.
...
### Exhausted waits are actionable, with one recorded attention change

Use one SQL/Core classifier for release/publication waits, current list/detail rows, top-level child aggregation, counts, command availability and progress recording. Its closed derived mode is automatic | recheck_required. Expose waitRecoveryMode on the strict waiting wire variant; forbid it on other variants. This is a projection, not a stored lifecycle field.

Automatic means an already-admitted unfinished recovery read or an existing admissible initial/event/Reconcile recovery opportunity remains for the exact obligation. Its next I/O may be delayed by a valid bound/date gate. A normalization-only wake, an expired or consumed grant, or the possibility of a future event is not automatic capacity. With no such opportunity, persist the waiting hold with NULL I/O due and project recheck_required/needs_decision. A current exact release/publication event can restore capacity later; the ticket must not promise that it will. Other holds, especially unresolved_write, retain their stronger recovery rules.

The row itself shows “Automatic release checks finished. Re-check readiness” or “Automatic publication checks finished. Re-check publication,” with Re-check now. Keep approved status and dated prerequisite/publication facts separately visible. Apply the same category to root/child counting and Mission Control's existing month placement; snooze/archive rules stay authoritative. Needs_decision here asks for bounded inspection, not another approval or permission to mutate.

Reuse action_command's existing proposed progress_exhaustion_reason, progress_step_id and progress_subject_attempt_id. On record_progress for blocked/awaiting_release with recheck_required: reason=binding_observation_budget, exact binding step, subject=NULL. On blocked/awaiting_publication with recheck_required: reason=readback_observation_budget, exact verification step and original-write subject. Both automatic waiting variants require all three NULL. The current mode is derived from locked facts; the immutable command records the reason known at transition, so later grants cannot rewrite its history.

Progress deduplication compares phase/result/hold AND the exact nullable reason/step/subject tuple against the latest progress command for that execution/action ordered by target result_version. A new required-recheck snapshot increments workflow version and attention_version once through record_progress; update its directly affected ticket and valid current-membership parent under the existing target guards. Every user's action_read remains unchanged. Duplicate polls, sync offers, normalization wakes and command replays append nothing and increment nothing. The reason labels mean no applicable capacity, including a decisive grant closure, not necessarily that every numeric counter reached its maximum. Derive each progress idempotency identity from the prior immutable progress command identity and the new closed signature through the canonical command encoding; a replay stays identical while a later cycle is distinct. Order by the execution ticket target version, never tied timestamps.

Before admitting an applicable Reconcile/event grant, compare the latest immutable progress snapshot with current capacity under the same captured DB time and locks. Materialize any pre-existing unrecorded exhaustion first as ordinary attention-bearing record_progress, then admit the grant and record restoration through the normal CAS/target chain; a grant that beats the worker wake cannot erase that attention cycle or fail merely because normalization was pending. When that accepted grant restores an exhausted waiting obligation, retain blocked with its same awaiting_release/awaiting_publication hold and non-null bounded due until the read is claimed. In that grant transaction, record a separate system/worker record_progress snapshot clearing those three cause fields. This narrowly defined transition increments workflow version once but preserves attention_version; it must follow a recorded recheck_required snapshot for that same waiting obligation. A poll cannot construct it. On the execution ticket, require its immediately preceding accepted target to be the exact newly admitted granting Reconcile/resume_hold command for this scope, and chain targets under the existing command-at-commit protocol. If that grant command itself increments workflow version, the progress record is a second explicit increment. Parent targets follow the validated child transition and membership guards; they need not have the child grant as their own preceding command. This durable automatic snapshot ensures that a later exhausted cycle is a new transition, rather than being suppressed as equal to the previous cycle. Do not reopen/reset the earlier grant.

Replace guard_progress_attention's unconditional +1 with three exhaustive branches: validated waiting-capacity restoration has delta 0; the narrowly validated schedule-caused same-wait normalization below has delta 0; every other newly recorded meaningful progress transition has delta 1. Both have workflow-version delta 1, exact before/after command targets, unchanged protected workflow fields, and existing tenant/current-approval/parent checks. No generic caller-chosen attention delta is introduced. This is distinct from schedule/unschedule, which still preserve attention for every actor.

Count exhaustion or decisive closure records recheck_required in the same finalization transaction. Wall-clock exhaustion is materialized by the existing worker's indexed normalization wake; GET/list/count never write workflow or personal-read state. Before that wake, read projections may identify exhausted capacity from their common database asOf; the subsequent progress comparison uses the prior immutable command snapshot, not a recomputation of yesterday's mode with today's clock, so the unread transition is not lost. List/count predicates use the same classifier, one captured database asOf and one scoped SQL statement or repeatable-read transaction for page plus counts; a shared timestamp alone cannot fix divergent READ COMMITTED snapshots, and a cursor does not freeze later HTTP requests; the normal bounded worker scan records attention independently of a user opening the detail.

If schedule/unschedule itself changes feasible capacity while retaining the same waiting hold and exact obligation, normalize that mode change and append its separate attention-neutral record_progress in the scheduling transaction. Compare before/after at the same DB time, including whether the new date leaves any permitted start before a fixed grant deadline; do not pretend capacity remains until a later timer if the new date already makes every start impossible. New cause fields reflect the resulting mode, while the schedule command keeps all progress fields NULL. Do not defer this schedule-caused change for a later unread-producing worker transition. A pre-existing unrecorded expiry is ordinary attention-bearing progress and must be normalized before this exception, using the normal locks and command CAS chain; scheduling cannot mask it. A changed hold is a separate prerequisite transition, not this silent exception.

Both attention-neutral branches require independent same-transaction admission proof from the canonical locked grant/schedule-and-progress entry point and immediate SQL guard. For the execution ticket, the preceding accepted grant/schedule target result_version must equal the progress target previous_version, which preserves durable historical attribution. That adjacency alone, matching timestamps or a user-settable session variable does not prove same-transaction admission and cannot authorize the exception. An unrelated later poll or historical command must fail. Parent scope remains checked through the valid child transition. A genuine later deadline expiry/provider result still gets one ordinary attention increment; no-op scheduling can neither mint a grant nor conceal consumed starts.

Required fixtures: due waiting→same hold with no capacity→needs_decision plus one attention increment; publication variant; current-member child→one root row/count/unread update; duplicate/concurrent normalization; time expiry before a user opens the inbox; mark-read before/after the transition without shared-state overwrite; Reconcile/event→automatic snapshot with no attention increment→second exhaustion with exactly one increment; grant/normalization race; schedule-caused mode normalization recorded atomically without attention, forged silent transitions refused, and unrelated later progress still attention-bearing. Also prove grant admission beats an unrecorded expiry wake without losing the exhaustion update, exhausted NULL-due plus missing progress cannot commit, migration activation rejects an unrepaired fixture, and the permitted due scan discovers every pending normalization. UI and list/count assertions must use the same data, not independently stubbed statuses.
...
For policy 2, an initial readback grant of an uncertain play_inspection_edit_delete reserves one of its existing permitted starts for a read after the exact inspection edit's stated expiry. No new grant kind, attempt flag, table or column is added. Resolve these immutable inputs:
...
V13 explicitly pins this cleanup policy-2 profile to N=6, O=168 hours, B=1 hour, G=24 hours, with the existing factor 2. This is a proposed operation-specific contract, not a claim that the prototype implements these fields or that all provider operations share them. A later shorter policy remains a separate versioned choice. Admission requires finite E and exact edit/receipt identity; missing or conflicting historical expiry is an explicit unsupported-recovery condition, never a guessed TTL or a renewed older-policy grant. Duration arithmetic must be finite and range-checked identically in SQL/Core.

Compute from those immutable facts:
...
The reserved read is scheduled promptly after the provider-stated expiry plus B, or after F+B if expiry already passed. Its fixed admission interval is [tail_due, initial_end). It is not placed exactly at an exclusive deadline and is not delayed merely to use the rest of the ordinary window. The grace is bounded by the already-pinned ordinary deadline and G; it is not a new relative window calculated when a worker wakes. The total wait depends on the provider's recorded absolute expiry, which is not asserted to have a universal TTL.

Before tail_due, an initial readback may start only under the ordinary backoff/window rule before ordinary_end and while fewer than N−1 original initial starts exist. At/after tail_due, at most one initial start is admitted before initial_end; unused ordinary slots are forfeited, not converted into extra tail retries. Derive tail consumption from original initial readbacks admitted at/after tail_due. Count failed, claim-expired and cancelled starts. Serialize selection/count/admission on the execution lock and retain the one-unfinished-attempt rule. No start is admitted at/after initial_end; a failed tail consumes its slot. An earlier valid quiescence/acknowledgement can close recovery immediately and makes the reserved read unnecessary.

Use one SQL/Core next_observation_start_at(scope,DB_now) selector returning the earliest permitted start or NULL. It respects the exact grant, backoff, authority, current claim and ordinary/tail intervals. Remaining schedulable capacity is different from admission right now: the reserved future start is capacity even after all N−1 early starts are used or ordinary_end passed. If no ordinary start fits, calculate tail_candidate=max(tail_due, applicable backoff_not_before, DB_now) for the unconsumed reservation and return it only when tail_candidate < initial_end. A delayed ordinary read cannot bypass its applicable backoff. If no tail start fits, retain the fixed initial_end state-normalization wake and evaluate other applicable grants; do not invent an immediate I/O loop or a start outside the interval. Other independently valid grants are evaluated separately; explicit Reconcile never renews this initial grant or silently consumes its reservation.

Table A retains uncertain/uncertain_write with non-null next_run_at at the next allowed opportunity while this initial reservation or another grant remains. The normalizer uses initial_end, and it cannot discard the future tail at the earlier F+O or ordinary_end boundary. The earlier required I/O/state-normalization wake stays durable in existing next_run_at and is recovered through the existing worker scan/queues. A live claim is handled first: an ordinary read still in flight can delay the tail until finalization/claim expiry, and an admitted tail read may finish after the start deadline. A late response follows the original readback's existing late-evidence path; it consumes no additional start.

If the tail is consumed without resolution or its fixed admission interval has expired, and no other applicable grant retains capacity, use blocked/uncertain_write, NULL due, retained exclusion and visible bounded Reconcile. An independently valid Reconcile grant keeps its own permitted due and recovery state; exhausting the initial tail cannot discard it. An exact-present response after the stated expiry still means mismatch/FALSE; unavailable, malformed, throttled or denied reads do not prove absence. A timer, expiry date and exhausted count never make a write final. This guarantees a durable bounded opportunity to observe, conditional on worker/provider availability within the interval; it cannot guarantee successful native readback or release at a wall-clock deadline. No safe-resend exception is introduced.

The provider documents an AppEdit's absolute expiry and that GET returns its representation. Those facts support scheduling and complete-present classification; the existing strict invalid/absent decoder still supplies quiescence evidence. Sources: [AppEdit resource](https://developers.google.com/android-publisher/api-ref/rest/v3/edits), [GET edit](https://developers.google.com/android-publisher/api-ref/rest/v3/edits/get).

Required cases: crash before send→uncertain→present mismatch reads→reserved opportunity survives early-count exhaustion→native invalidity after expiry→quiescence and release without manual Reconcile; expiry already past or beyond F+O; missing/mismatched anchor; worker within versus at/after deadline; tail admission then crash; later GET expiry cannot extend the anchor; earlier success cancels the wake; concurrent Reconcile/late ACK and stale jobs; post-expiry present/unreadable evidence remains uncertain. These are required fixtures, not executed tests.
...
The I/O due lower bound is the greatest of command admission, original Undo deadline, new schedule if any, and the recorded policy backoff. For obligations outside the same-wait scheduling exception, persist the earlier required deadline-normalization wake when that lower bound falls outside the existing grant window; that wake changes state only and cannot bypass the date to perform I/O. For an unchanged awaiting_release/awaiting_publication hold whose schedule change itself makes every start infeasible or restores feasible capacity, normalize the wait mode and its attention-neutral progress snapshot immediately in the scheduling transaction as specified above, rather than deferring that mode change until the grant deadline. Normalize any pre-existing unrecorded exhaustion before applying that exception. Do not reuse an old claim expiry as backoff. The no-effect exhausted case offers Reconcile/Edit; event recovery additionally follows the exact binding scope rule below. No new grant or approval is created by scheduling.
...
- record_progress: accepted system/worker; progress_execution_id and progress_phase required; phase restricted to verification_due/uncertain/blocked/settled; result present exactly for settled; hold present exactly for blocked/uncertain; all resume fields NULL. The scope/reason fields follow the exact retry_exhausted and recheck_required waiting matrix above; automatic waiting progress and every other hold/result require all three NULL. The narrowly validated automatic-capacity-restored waiting transition and the validated schedule-caused same-wait mode transition both record history/version without attention; every other newly recorded meaningful progress transition increments attention once. Each exception requires its exact cause, scope and same-transaction admission proof described above.
...
Policy-2 cleanup readback uses the exact expiry-tail profile and selection rules above; do not infer that contract from binding-inspection defaults. All non-write kinds set terminal_on_acknowledgement=false; “not applicable” is not a nullable stored value. Policy values are fixed by a closed recovery-policy version, not caller-selected knobs. Initial inspection policy remains six starts in 168 hours, backoff one hour doubling to a 24-hour cap; prewrite uses the same bounded policy unless a future explicitly versioned operation contract changes it.
...
An execution already in its correct exhausted hold with NULL due and matching latest immutable progress snapshot has no wake until new evidence/grant/state arrives. Make their atomic persistence a commit-time invariant: an exhausted awaiting_release/awaiting_publication row cannot commit NULL next_run_at unless the latest progress snapshot matches its exact required-recheck scope, cause and resulting execution-ticket target version, with the required current parent targets. Validate that snapshot's immutable target chain; later unrelated command versions need not equal its recorded result_version. Finalization, grant admission, schedule admission and normalizer paths must materialize any missing snapshot under the same locks before clearing the due; a still-pending normalization remains discoverable through a non-null allowed due. Migration/backfill must repair or explicitly quarantine pre-existing violations before enabling this guard and activating the due scan; activation assertions must reject unresolved eligible NULL-due/missing-snapshot rows. GET does not repair them, and no unindexed or imaginary wake is relied upon. This does not add a queue engine or API timer.

On this wake, acquire the normal locks and **apply the tables without admitting I/O**, without inventing an observation and without resetting a count/window. Persist the resulting hold/phase/NULL due, clear any no-longer-live claim through the existing recovery rules, increment schedule generation and record a deduplicated attributable progress transition when its visible state changed. An unchanged hold can still have a changed automatic/recheck_required mode; record that exact transition using the reason/scope snapshot and attention rules above. An unchanged full projection or a poll produces no extra unread change. Only then does ordinary delivery eligibility become NULL. The queue never needs an impossible final readback merely to reveal exhaustion.

If the calculated next backoff crosses the last eligible window end, schedule the normalization wake at that window end, not beyond it. For the reserved cleanup-tail scope, use its derived initial_end and next permitted tail start; the ordinary boundary is not total-grant exhaustion. Count exhaustion is normalized in the finishing transaction; deadline expiry is normalized by the existing worker scan. Preserve the constraints requiring non-null due times for ready, verification_due and uncertain.
