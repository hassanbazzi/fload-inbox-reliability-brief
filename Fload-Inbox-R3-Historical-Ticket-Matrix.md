...
Source prototype ticket-service.ts:363–374 includes that failed non-perform branch. Do not silently remove it through the review's shorter declined/acknowledged predicate. Failed perform work still requires its separate accepted reopen/effect protocol. No caller may select placement on a preimage whose old restore changes decision. Missing mode refuses under the new strict contract; clients and source/digest versions must migrate together rather than backfilling guessed intent on old immutable commands. Historical prior-source restore labels remain claims. Restore preserves `attention_version` and creates no provider I/O, proposal or approval.

**Admission and restore (D3):** slot classification is independent of `attention_version`; use the [single admission matrix and guard order](Fload-Inbox-R3-Parallel-Work-Coordination.md#manual-commands-and-historical-adoption-d3). Historical placement/unhide/reconsideration remains historical and consumes zero slots. Work declined/acknowledged→open is new discretionary admission if it raises occupied slots, including for a human; at capacity it returns the proposed closed `client_attention_capacity` refusal without changing decision. The existing approved+failed non-perform branch is recovery when the exact current execution/approval meets its guards, so capacity cannot refuse it. Count-affecting restore/Undo/recovery takes canonical idempotency locks, then the shared sorted organization/app admission guards, then ordered action/domain locks, before classification and persistence. Do not acquire the admission guard after holding an action lock. Existing-work exceptions still serialize their count change with new admissions; placement and personal read state do not manufacture spare capacity.

### 2.2 Canonical historical baseline capture (M1, qualified)

Allow accepted record_observation on historical/open for adoption baseline preparation. Also allow a **conditionally reachable source-change capture** on historical/declined for the accepted automatic replace_declined path only when the rejected source can be independently recovered and compared. Declined capture requires the exact currently effective reject command/head and a proved comparable original source; unknown legacy preimages remain suppressed. Neither capture changes record kind, decision, display head, approval or suppression. They increment workflow `version` once and preserve `attention_version` because they only prepare non-actionable evidence. Historical/superseded and historical_deleted/* refuse both captures. No write, generation or execution is created.
...
**Unread counter rule (C5/D1):** the canonical writer derives `attention_version` changes from prior state, new state and cause under ownership v6. These two evidence-only historical captures increment workflow `version` once and `attention_version` by zero, and never advance personal read marks. Do not copy v15:747's broad “ordinary observation version/attention” wording into these additional branches. A subsequent adopted revision or meaningful visible transition receives its own normal `attention_version` treatment; evidence capture cannot silently perform that transition. The existing failed-perform refresh branch is not redefined here. The five-item cap never reads `attention_version` or a personal read watermark; an attention slot means a current client-action obligation, not a counter increment.
...
2. Under canonical idempotency locks, then sorted per-(organization, app) admission guards before the existing ordered action/domain locks, re-read authority, exact kind/decision/version/head, source identities and dependencies. Require historical/open, NULL approval, no approval/execution rows, and the durable cutover disposition showing no unresolved prior effect. Revalidate the prepared baseline under the same required freshness/target rules as new work. A concurrent source/head/permission change refuses atomically. Apply the coordination note’s canonical slot classification: this historical→work activation is new discretionary admission even for a present human. Any positive slot delta requires headroom at the locked database asOf; full capacity records the proposed `client_attention_capacity` refusal without changing the historical kind/head or suppression. Baseline capture already committed as a separate evidence command remains valid evidence; refusal does not turn it into approval.
3. Append and seal the new proposal through the existing closed domain path. Where its domain/operation requires a baseline, pin the already persisted qualifying observation as baseline_revision_id. For request/advisory operations without a baseline contract, do not invent an observation or perform provider I/O. The accepted revise target records previous_revision_id=the exact historical head and result_revision_id=the new proposal under contracts §2.4; no new adoption column or generation provenance is needed. Change **record_kind historical→work**, preserve decision=open, set the proposal as current head, leave current_approval_id NULL, and increment `version` and `attention_version` once as a revision change. Keep action ID, creation key, structural parent, all aliases, historical revisions, rejection claims and actors.
...
Use the already accepted v15 `revise/replace_declined` mode, not the edit branch. It may transition historical/declined→work/open only with exact current pins, a proved meaningful review-source change in rating/body/title/provider-edited flag, a fresh source baseline, and no unresolved effects (v15:183–185). Preserve the original suppression/rejection history; use the carried source-change idempotency key. Unknown legacy preimages and manual authorship are not substitutes for the meaningful-change proof. This branch also creates no approval. Like explicit historical adoption, it takes the admission guard before action locks and requires room for a positive slot delta; a proved source change supplies no cap exemption. Its stable source-change key is deterministic, however, so full capacity uses the [canonical producer deferral protocol](Fload-Inbox-R3-Parallel-Work-Coordination.md#deterministic-producer-keys-defer-before-command-creation-f2), not an immutable client_attention_capacity refusal under that key. Resolve any existing receipt/digest first, then evaluate capacity inside the final locked eligibility transaction before inserting the command. A capacity-only deferral writes no command/target/proposal/approval/execution and consumes no key; the historical head and active decline remain unchanged. The worker's SQL-backed producer scan must re-offer the still-eligible source change with the same key after a slot opens, even after queue loss or a long wait. Revalidate fresh pins, baseline and full meaningful-change proof; do not append an epoch or fabricate an execution to make it discoverable. Caller-minted edit attempts retain the ordinary immutable refusal/new-key retry contract.
