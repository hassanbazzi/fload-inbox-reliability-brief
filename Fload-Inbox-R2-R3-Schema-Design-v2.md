...
**Review update:** the independent v2 review confirms the corrections and source inventory. Clarifications C1–C6 are applied below; the version stays v2. [Review disposition](Fload-Inbox-R2-R3-v2-Review-Resolution.md) records the changes and the next deliverable. This closes the clarification pass, not R2/R3, destination DDL or migration validation.
...
1. Acquire the accepted ordered locks and validate the canonical command idempotency key/digest **before semantic capture replay**. Reusing a key with different input refuses the command request as `idempotency_mismatch` (the existing command response has `outcome='conflict'`); a matching capture elsewhere cannot rescue that invalid key reuse. The original command and receipt remain unchanged. This is not validated provider evidence: it creates no `conflicting_completion`, capture leaves, diagnostic hold or attention change. For an admissible new request, resolve the original interaction and current state, validate its pinned contract, and compare against committed original, late and conflict captures **before creating a new evidence command or changing a hold**. Exact replay returns the existing capture. A distinct captured provider completion requires its own valid command admission.
...
3. A conflict cannot displace a newer live claim or unfinished attempt. Store the evidence without overwriting that attempt's fence, claim, phase or routing. Existing in-flight I/O must still be allowed to persist its result. The unresolved-conflict check immediately forbids new effects, plan expansion, output adoption (including the newer in-flight generation's output), successful execution settlement, guard release and ordinary reopen. The execution remains on the diagnostic path; this restriction does **not** suspend Usage settlement for incurred model calls. The next canonical transition reaches the diagnostic hold when the existing attempt is safely finalized/recovered. It preserves the original unfinished-attempt routing and claim-expiry recovery obligation while waiting. If that write expires without a captured definitive completion, normal fenced, database-only expiry classifies it `uncertain/claim_expired` and enters `blocked/evidence_conflict`; it does not perform an automatic pre-resend readback or another provider call. Readback/inspection expiry retains its own unreadable contract. Existing captured completions may still persist under the current/late rules; expiry never overwrites a definitive result.
4. With no live/unfinished interaction, a nonterminal execution may enter `blocked/evidence_conflict` through ordinary Core progress and attributable target/version handling. Only an actual user-facing change bumps attention; duplicate capture and additional evidence on the same unchanged hold do not repeatedly bump it. Diagnostic reads require the carried **explicit bounded Reconcile** authority for the exact historical scope; automatic initial/event capacity and the conflict record itself cannot authorize them. Such a read cannot clear the hold, settle the execution, reopen it, release exclusion or authorize an effect.
5. If already terminal, retain the contradictory fact in history without rewriting completion, reopening the execution, reacquiring/releasing its guard or settling Usage again. Extend the worker-side operational integrity monitor specified in ownership v6 §5.7 to detect terminal conflicting captures from their durable records independently of the ordinary due-execution scan. It uses bounded, fair scanning in the leased worker lifecycle and surfaces an operational alert with tenant, execution and capture identities through the existing ops-alert destination; duplicate observations use that stable identity for deduplication. Monitor interruption cannot delete the durable finding. This is an implementation obligation, not a claim that a monitor already exists. It causes no customer unread bump, ticket reopening, provider I/O or Usage change. A new provider read always has its own separately admitted original attempt and the required authority.
...
The migration principal is always the importer, never the historical approver. Missing actors remain unknown, not system. Positive source evidence of `initiator='system'` remains a known system initiator; `initiator='agent'` retains its distinct agent provenance. A policy-mode decision records a known system actor only when its actual source evidence proves that attribution; a missing user FK or a mode label alone cannot supply it. These historical initiator facts confer no current approval or execution authority. User erasure atomically changes the FK and retained actor state through the explicit erasure exception; ordinary immutability cannot silently block `SET NULL` or leave `actor_state=known` with no actor.
...
**Required baseline for newly executable imported work.** Choose the fresh-evidence branch: a historical snapshot does not satisfy a required baseline for sealing a new executable proposal, including `next_editable_release`. Import performs no provider I/O. Without qualifying fresh evidence, the source component remains an explicit migration blocker and its original sources/history are retained; do not commit an invalid ticket skeleton, an approvable revision or a synthetic execution. `baseline_required` and `incomplete_revision` name the carried v15 design validation outcomes, not new lifecycle states or claims about implemented migration errors (the prototype uses `requires_provider_capture` and `baseline_changed`).

A separate canonical read-only preparation obtains fresh, correctly scoped evidence outside the import transaction. Final source fingerprint, tenant, target and exact-head checks, complete baseline/proposal sealing, history and alias adoption commit atomically through the applicable canonical path. Preserve the same canonical ticket identity and aliases across this preparation; a later capture must not mint a second work identity. Missing, stale, incomplete or mismatched evidence keeps the component blocked from executable import and source retirement. Any permitted historical-incomplete record remains non-authorizing and must satisfy its separately defined history shape.

Declined review history and its suppression identity are preserved independently. A fresh baseline alone cannot reopen a declined ticket. `replace_declined` still requires meaningful source-fingerprint change, exact current pins and no unresolved effects, and installs the new baseline/proposal with reopening in the same transaction. Imported evidence never bypasses that rule.
...
| R2 concurrency | Identical/conflicting response in both lock orders; idempotency mismatch refused without capture/hold; existing original/late/conflict replay; stale fence and a newer active attempt; expiry under conflict without automatic readback; lost commit response; reservation readback manifest; actual Usage settles independently; terminal contradiction rediscovered by the ops monitor; erasure. |
| R3 conservation | Complete field and nested-variant destination ledger against a pinned current main; partial receipts, decisions/proofs, known system/agent initiator versus missing actor identity, source links, schedules, aliases and erasure scopes. |
| R3 migration | Duplicate-source consolidation; changed source between preflight/apply or baseline preparation/apply; required baseline missing without invalid skeleton; fresh baseline arriving after a declined source with no reopening; unresolved prior send; missing actor/membership; rejected snapshot; event completion links; multi-asset history; restart and source retirement. |
