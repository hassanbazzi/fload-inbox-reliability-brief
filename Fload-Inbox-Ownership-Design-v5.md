# FLO-1355 — ownership design v5: explicit routing and release-offer admission

18 September 2026 · Corrected documentation draft · Implementation paused

This draft applies C1–C2 from the ownership-design-v4 review, which confirms N1–N4 and the public overview. It supersedes ownership draft v4; published v15 remains the behavioural baseline where this draft explicitly carries its rules. This is a documentation proposal, not a description of shipped behaviour or complete executable DDL. R2/R3, full schema/type generation and implementation proof remain outstanding. Platform implementation remains paused.

The [v4 review resolution](Fload-Inbox-Ownership-v4-Review-Resolution.md) records the disposition and required verification. See the [visual comparison](ownership-overview.html), [previous ownership v4](Fload-Inbox-Ownership-Design-v4.md) and [v15 behavioural baseline](Fload-Inbox-Revised-Design-v15.md). The new visual comparison distinguishes the originally audited inbox, the earlier v15 proposal and this ownership design. The preserved prototype is a frozen audit baseline, not a fresh claim about production. No platform code, migrations or provider calls are changed by this documentation pass.
...
| `action_execution` | as v1 | `result` keeps `handled_externally` (O7). Existing `next_step_id` is the transactionally maintained next-obligation routing projection (§5.7), never execution authority by itself. |
...
**Offer before the initial cycle.** Under the same action/execution locks, a release offer for an obligation with no accepted initial `open_recovery` is ignored, even if its date gate is already open. It creates no accepted or refused command, consumes no event/idempotency identity, opens no cycle and changes no ticket state. The durable initial obligation remains due; its first admitted inspection reads current provider readiness. An offer racing that admission is evaluated against the committed state reached in lock order. Once a cycle exists, the ordinary exact-hold, authority, date, native-identity and deduplication rules still apply; a cycle's existence alone does not make an event eligible. Successful later syncs may offer the same resource again because the ignored offer consumed no identity. An ignored offer is not promised future capacity and does not reset any window.
...
Required traces: two ordinary commands in one org; rejected Re-check then accepted Re-check; duplicate initial opening; concurrent replacement; approve in September for November with a full initial window in November; first-admission rollback before commit and crash after commit; contention/support/date gates without budget use; Re-check after the original window expired; same event after supersession; in-flight replacement; immutable expiry despite later GETs; earlier/later protected deadlines; stale delivery and old-cycle late evidence; offer after the date gate but before the first cycle; offer/first-admission race in both lock orders; ignored offer later becoming applicable without prior identity consumption. These are design requirements until implemented and tested against actual DDL.
...
1. **Support before decoding and claims.** Read only neutral routing facts before decoding provider payloads or newer native variants. For a due execution E, let A be its unique original unfinished attempt (evidence-only `late_evidence` and `conflicting_completion` records never qualify). Define `route_step_id = A.step_id` when A exists, otherwise `E.next_step_id`. Before LIMIT, include the row only when that step belongs to the same organization/execution and its immutable `operation_contract_id` is in the build's supported manifest, or when `route_step_id IS NULL` and the operation is strictly state-only normalization under the closed neutral protocol. A missing non-NULL step or contract is an integrity error, not the NULL branch. Do not require support for already completed historical steps or unrelated future steps. The SQL query resolves only these references; it does not decode native content or duplicate Core's next-operation policy.

   The canonical writer must recompute the existing `next_step_id` with `next_run_at` whenever an accepted change can alter the next obligation: initial planning, binding/manifest append, commands and cycle changes, admission/finalization, and authoritative late evidence. The pointer names the next inspection, original step whose effect needs readback, cleanup, generation or write under Core's normal ordering; it is not the last step attempted or a promise that the next action is a write. An incomplete plan routes to its concrete continuation-owner step, not NULL. No new projection table or field is introduced. A late-evidence refresh cannot supersede an active attempt, change its fence or authorize an effect. This stronger invariant is proposed work: the preserved prototype does not maintain it on every path.

   NULL permits only state-only normalization; it never authorizes provider I/O, generation, attempt expiry or missing-plan continuation. If normalization establishes an I/O obligation, persist the step/due projection and return for ordinary supported admission. Direct queue deliveries use the same gate. Under action → execution → guard locks, revalidate current routing and exact operation support before any claim, attempt expiry, cycle opening or progress, and repeat the support check if transaction-local selection advances to a different operation. The unsupported candidate itself causes no claim, expiry, cycle or customer progress change. Completion of already-admitted supported S1 still persists its evidence and the S2 route/due time; discovering unsupported S2 must not roll back S1 completion. The worker then stops before S2 admission and leaves it due for a compatible build. Startup/cutover must rebuild and verify the projection while pre-gate writers are drained, before enabling this scan.
...
5. **Acceptance proof.** Mixed old/new workers; support-filtering beyond a page of unsupported rows; unknown contract before strict decoding; missing queue repeatable; owner drain; Redis interruption; sustained page backlog and bounded healthy-capacity rediscovery; expired claim after an uncertain write; S1 supported recovery with future S2 unsupported; unsupported historical S1 with supported current S2; readback routed to an earlier step; NULL normalization; missing reference; binding-plan append and late evidence changing the route; selection advancing to a new contract within one transaction. Assert no unsupported claim, hold, budget renewal, guard release or resend. The existing rediscovery/recovery targets remain targets until measured under representative load.

No additional SQL heartbeat/capability table is proposed. A live-worker lease is not proof of execution capability or lack of it. The deployment manifest and operational health expose coverage; the support gate and compatible local sweep provide execution correctness and recovery. The current prototype does not implement this mechanism or the complete routing-projection invariant yet.
...
9. V5 makes routing and early-offer admission explicit without changing the accepted workflow semantics. Unrelated unsupported steps do not block supported recovery; an offer before the initial cycle grants and consumes nothing.
...
- **Supported recovery, unsupported successor.** S1 uses contract v2 and has an unresolved issued effect; S2 pins v3 and has not run. A v2 worker can inspect S1. Once S1 recovery finishes, the writer moves the route to S2 and the v2 scan excludes it before LIMIT. Completed unsupported historical steps likewise do not exclude supported current work.
- **Offer beats first inspection.** The date gate opens at 09:00; an offer at 09:00:30 finds no initial cycle and consumes no event identity. Admission at 09:01 atomically opens C0 and reads current readiness. Later offers remain subject to exact-hold eligibility and ordinary deduplication.
